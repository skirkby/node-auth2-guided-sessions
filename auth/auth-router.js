...
// points to a session record in the store, which contains a value that could
...
// if it has been modified.) So the act of setting req.session.loggedin will
// result in a session object being created, and a set-cookie header being added
// to the response. This is true even if req.session.loggedin is set to false!
// We must NOT modify req.session *unless* we are certain that we want a session
// record created. Reason out what would happen if we set req.session.loggedin =
// false, and the user then failed to authenticate... and then tried to
// authenticate again (and did so successfully.) What would happen? (Good
// thought exercise...)
// 
...
// handler.
//
// On successful login, we not only send a 200 back, but we also store
...
// or "active". In addition, modifying req.session will cause it to be saved as
// a session record (together with any data we added to it), which will be
// stored in our store. AND, doing that will cause express-session to add a
// set-cookie header on our response, causing the browser to 1) store the cookie
// value (which is an encrypted form of the session ID), and 2) include it in a
// cookie header in subsequent requests (until the cookie expires). That way,
// every subsequent request has a value that express-session can attempt to
// decrypt, and if it succeeds, it can then check the session store to see if it
// is for a valid session, and if it is, it can add the session record from the
// store to the req.session object. That way, our restricted middleware can
// check to see if the req.session object is the default vanilla one, or if it
// came from our store, which means that a valid session ID was sent in by the
// browser. Phew. Taking a nap now. 
//----------------------------------------------------------------------------//
router.post('/login', (req, res) => {
  let { username, password } = req.body;

  // In a previous version of this project, I included the following line.
  // However, I have learned that express-session will store a session record
  // and send a set-cookie header if we make ANY changes to req.session, even
  // this initializing change. So we DO NOT want to include it. (Doing so will
  // force a new session record to be created, which will fail our restricted()
  // middleware test, and will cause a set-cookie header to be sent, which will
  // point to this useless session record.) This isn't any less secure, but it
  // just has the potential to fill up our database with cruft that we don't
  // need, and could potentially expose us to a DOS attack (Denial Of Service) -
  // someone could just repeat failed authentication requests, and each one
  // would add a session record to our DB. After a billion or so of those, our
  // DB might fall over... We don't want to mess with req.session unless we are
  // sure we want a session record. It pays to read the documentation! See
  // https://www.npmjs.com/package/express-session, under the section
  ...
  Users.findBy({ username })
    ...
    .then(user => {
      if (user && bcrypt.compareSync(password, user.password)) {
        ...
        // req.session.loggedin not exist if we end up here... see above... so
        // we are not in danger of a session being created - we just don't
        // modify req.session, and we are good.
        res.status(401).json({ message: 'Nice try. But, no. Try. Try again.' });
      }
    })
    .catch(error => {
      // req.session.loggedin not exist if we end up here... see above... so we
      // are not in danger of a session being created - we just don't modify
      // req.session, and we are good.     
      res.status(500).json(error);
    });
});
